...
async function main() {
  ...
  const eng1 = await prisma.engagement.create({
    data: { clientId: client1.id, type: "SECRETARIAL", fiscalYear: "2025", status: "ACTIVE" },
  });
  const eng2 = await prisma.engagement.create({
    data: { clientId: client1.id, type: "ACCOUNTING", fiscalYear: "2025", status: "ACTIVE" },
  });
  const eng3 = await prisma.engagement.create({
    data: { clientId: client2.id, type: "TAX", fiscalYear: "2025", status: "ACTIVE" },
  });

  // Create sample cases
  await prisma.case.create({
    data: {
      caseNumber: "SEC-AR-2026-001", clientId: client1.id, engagementId: eng1.id,
      ownerId: secStaff.id, title: "Acme Trading - Annual Return 2026",
      description: "Annual return submission for Acme Trading Sdn Bhd",
      department: "SECRETARIAL", caseType: "AR", status: "AWAITING_CLIENT",
      priority: "HIGH", dueDate: new Date("2026-01-15"),
      anniversaryDate: new Date("2026-01-15"), paymentStatus: "PENDING",
      outstandingAmount: 1500, signedCopyReceived: false, originalSignedReceived: false,
    },
  });

  await prisma.case.create({
    data: {
      caseNumber: "SEC-FS-2025-001", clientId: client2.id, ownerId: secStaff.id,
      title: "Bright Solutions - Financial Statements 2025",
      department: "SECRETARIAL", caseType: "FS", status: "ACTIVE", priority: "NORMAL",
      dueDate: new Date("2026-06-30"), financialYearEnd: new Date("2025-06-30"),
  ...
  await prisma.case.create({
    data: {
      caseNumber: "ACC-ENT-2025-001", clientId: client1.id, engagementId: eng2.id,
      ownerId: accStaff.id, title: "Acme Trading - Accounting FY2025",
      department: "ACCOUNTING", caseType: "ACCOUNTING", status: "ACTIVE", priority: "NORMAL",
      financialYearEnd: new Date("2025-12-31"), paymentStatus: "PENDING", outstandingAmount: 3000,
    },
  });

  await prisma.case.create({
    data: {
      caseNumber: "TAX-FIL-2025-001", clientId: client2.id, engagementId: eng3.id,
      ownerId: taxStaff.id, title: "Bright Solutions - Tax Filing 2025",
      department: "TAX", caseType: "TAX_FILING", status: "AWAITING_CLIENT", priority: "HIGH",
      dueDate: new Date("2026-07-31"), paymentStatus: "OVERDUE", outstandingAmount: 2500,
    },
  });

  await prisma.case.create({
    data: {
      caseNumber: "DEB-REC-2025-001", clientId: client3.id, ownerId: accStaff.id,
      title: "Golden Star - Outstanding Payment Recovery",
      department: "ACCOUNTING", caseType: "DEBT_RECOVERY", status: "ACTIVE",
      priority: "URGENT", paymentStatus: "OVERDUE", outstandingAmount: 8500,
    },
  });
  ...
  await prisma.reminderTemplate.createMany({
    data: [
      {
        name: "AR 1st Reminder", department: "SECRETARIAL", channel: "EMAIL",
      ...
      {
        name: "AR 2nd Reminder", department: "SECRETARIAL", channel: "EMAIL",
        subject: "URGENT: Annual Return Overdue - {{clientName}}",
        body: "Dear {{contactName}},\n\nWe note that the annual return for {{clientName}} was due on {{dueDate}} and we have not received the required documents and/or payment.\n\nPlease take immediate action to avoid late filing penalties.\n\nBest regards,\n{{staffName}}",
        reminderType: "AR_2ND",
      },
      {
        name: "Payment Reminder", department: "ACCOUNTING", channel: "EMAIL",
      ...
      {
        name: "FS 1st Reminder", department: "SECRETARIAL", channel: "EMAIL",
  ...
  await prisma.reminderPolicy.createMany({
    data: [
      {
        name: "Standard AR Reminder Policy", department: "SECRETARIAL", caseType: "AR",
        ...
        stopConditions: JSON.stringify(["DOCUMENTS_COMPLETE", "PAYMENT_CONFIRMED", "ORIGINAL_RECEIVED", "CASE_ARCHIVED"]),
      },
      {
        name: "Standard FS Reminder Policy", department: "SECRETARIAL", caseType: "FS",
        cadence: JSON.stringify([
      ...
      {
        name: "Debt Recovery Reminder Policy", department: "ACCOUNTING", caseType: "DEBT_RECOVERY",
        cadence: JSON.stringify([
